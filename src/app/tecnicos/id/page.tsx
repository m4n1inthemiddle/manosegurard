...
export default function TecnicoPage({ params }: { params: { id: string } }) {
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
          {technician.photo_url && (
            <div className="relative w-32 h-32 rounded-full overflow-hidden">
              <Image src={technician.photo_url} alt={technician.name} fill className="object-cover" />
            </div>
          )}
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-3xl font-bold">{technician.name}</h1>
            <p className="text-blue-600 text-lg">{technician.category}</p>
            <p className="text-gray-600">{technician.province} - {technician.sector}</p>
